...
public class DBHelper extends SQLiteOpenHelper {
    ...
    private static final String CREATE_CONTENT_SQL = "create table if not exists " +
    ...
    private static final String CREATE_LOG_DRAFT_SQL = "create table if not exists " +
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL(CREATE_LOG_DRAFT_SQL);
        db.execSQL(CREATE_CONTENT_SQL);
        Log.i(TAG, "onCreate: " + CREATE_LOG_DRAFT_SQL);
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
        if (oldVersion == 1 && newVersion == 2) {
            db.execSQL(CREATE_LOG_DRAFT_SQL); //添加草稿箱
        }
    }
}
